...
import { useNavigate } from "react-router-dom";
import { AuthContext } from "../../context/AuthContext";
import { FiArrowRight, FiArrowLeft, FiMenu } from "react-icons/fi";
...
const CATEGORIES = [
  ...
  { label: "Гастрономия", color: "#f59e0b", bg: "#fffbeb" },
  { label: "История", color: "#b45309", bg: "#fef9c3" },
  { label: "Пляжный отдых", color: "#0ea5e9", bg: "#eff6ff" },
  { label: "Горы", color: "#64748b", bg: "#f8fafc" },
  { label: "Приключения", color: "#ef4444", bg: "#fef2f2" },
  { label: "Архитектура", color: "#7c3aed", bg: "#f5f3ff" },
  { label: "Фототуризм", color: "#0891b2", bg: "#ecfeff" },
  { label: "Экотуризм", color: "#15803d", bg: "#f0fdf4" },
  { label: "Активный отдых", color: "#b91c1c", bg: "#fef2f2" },
  { label: "Фестивали", color: "#f97316", bg: "#fff7ed" },
];

const DEST_COLORS = [
  "#6366f1","#0ea5e9","#f59e0b","#16a34a","#ef4444","#8b5cf6",
  "#f97316","#ec4899","#0891b2","#7c3aed","#b91c1c","#0d9488"
];
  ...
  return (
    <div className="home-page">
      ...
      <aside className={`sidebar ${sidebarOpen ? "open" : ""}`}>
        <h3 className="sidebar-title">Travel</h3>
        <nav className="sidebar-nav">
          <button onClick={() => navigate("/traveler/my-routes")}>
            <div className="icon-box" /> Мои маршруты
          </button>
          <button onClick={() => navigate("/traveler/create-route")}>
            <div className="icon-box" /> Создать маршрут
          </button>
          <button onClick={() => navigate("/traveler/search")}>
            <div className="icon-box" /> Найти маршруты
          </button>
          <button onClick={() => navigate("/traveler/recommended")}>
            <div className="icon-box" /> Рекомендации
          </button>
          <button onClick={() => navigate("/traveler/favorites")}>
            <div className="icon-box" /> Избранное
          </button>
          ...
          <button onClick={() => navigate("/traveler/profile")}>
            <div className="icon-box" /> Профиль
          </button>
        </nav>
        <button className="sidebar-logout" onClick={onLogout}>
          <div className="icon-box" /> Выйти
        </button>
      </aside>
      {sidebarOpen && <div className="overlay" onClick={() => setSidebarOpen(false)} />}
    ...
    <FiMenu size={24} />  {}
        ...
        <section className="thp-hero">
          <div className="thp-hero__content">
            <h1 className="thp-hero__title">Найди своё идеальное путешествие</h1>
            <p className="thp-hero__sub">Тысячи маршрутов от профессиональных гидов</p>
            <div className="thp-quick-tags">
              <span>Популярно:</span>
              {["Европа", "Грузия", "Горы", "Гастрономия", "Природа"].map(tag => (
                <button key={tag} className="thp-tag" onClick={() => goToCategory(tag)}>
        ...
        {!loadingStats && (
          <section className="thp-stats">
            <div className="thp-stat-item">
              <span className="thp-stat-item__num">{recentRoutes.length > 0 ? `${recentRoutes.length}+` : "0"}</span>
              <span className="thp-stat-item__label">маршрутов от гидов</span>
            </div>
            <div className="thp-stat-divider" />
            <div className="thp-stat-item">
              <span className="thp-stat-item__num">{(stats?.destinations ?? []).length || "0"}</span>
              <span className="thp-stat-item__label">направлений в БД</span>
            </div>
            <div className="thp-stat-divider" />
            <div className="thp-stat-item">
              <span className="thp-stat-item__num">{(stats?.themes ?? []).length || "0"}</span>
              <span className="thp-stat-item__label">категорий интересов</span>
            </div>
            <div className="thp-stat-divider" />
            <div className="thp-stat-item">
        ...
        <section className="section thp-section">
          <div className="section-header">
            ...
            <div className="controls">
              <button onClick={() => scroll(destRef, "left")}><FiArrowLeft size={20} /></button>
              <button onClick={() => scroll(destRef, "right")}><FiArrowRight size={20} /></button>
            </div>
          </div>
          <div className="horizontal-scroll thp-dest-scroll" ref={destRef}>
            {(stats?.destinations ?? []).length === 0 ? (
              ...
              <div
                ...
                title={`Найти маршруты: ${d.title}`}
              >
                <div className="thp-dest-card__emoji">
                  <div className="icon-box" />
                </div>
                <div className="thp-dest-card__info">
                  <span className="thp-dest-card__city">{d.title}</span>
                  <span className="thp-dest-card__country">{d.price || "маршруты"}</span>
                </div>
                <div className="thp-dest-card__arrow"><FiArrowRight /></div>
              </div>
            ))}
          ...
          <div className="thp-cats-grid">
            {CATEGORIES.map((cat, i) => (
              <button
                ...
                title={`Маршруты: ${cat.label}`}
              >
                <span className="thp-cat-card__emoji"><div className="icon-box" /></span>
                <span className="thp-cat-card__label">{cat.label}</span>
                <div className="thp-cat-card__arrow"><FiArrowRight /></div>
              </button>
            ))}
          </div>
        </section>
        ...
        {recentRoutes.length > 0 && (
          <section className="section thp-section">
            <div className="section-header">
              ...
              <div style={{ display:"flex", gap:8, alignItems:"center" }}>
                <div className="controls">
                  <button onClick={() => scroll(recentRef, "left")}><FiArrowLeft size={20} /></button>
                  <button onClick={() => scroll(recentRef, "right")}><FiArrowRight size={20} /></button>
                </div>
                <button className="thp-see-all" onClick={() => navigate("/traveler/search")}>
                  Все маршруты <FiArrowRight />
                </button>
              ...
              {recentRoutes.map((r, i) => {
                ...
                return (
                  ...
                  >
                    ...
                    {r.guideFullName && (
                      <div className="thp-route-card__guide">
                        <div className="icon-box" />Гид:  {r.guideFullName}
                      </div>
                    )}
                    <div className="thp-route-card__meta">
                      {startDate && (
                        <span><div className="icon-box" /> {new Date(startDate).toLocaleDateString("ru-RU", { day:"numeric", month:"short" })}</span>
                      )}
                      {r.durationDays && <span>{r.durationDays} дн.</span>}
                    </div>
                    <div className="thp-route-card__cta">
        ...
        <section className="section thp-section thp-how">
          ...
          <div className="thp-how__steps">
            {[
              { step:"01", title:"Ищи маршруты", desc:"Выбери направление, транспорт и даты. Найди маршрут от профессионального гида или другого путешественника." },
              { step:"02", title:"Сохраняй в избранное", desc:"Добавляй понравившиеся маршруты в избранное чтобы не потерять." },
              { step:"03", title:"Путешествуй", desc:"Открой маршрут, изучи точки и отправляйся в путь! Или создай свой собственный маршрут." },
            ].map((s, i) => (
              <div key={i} className="thp-how__step">
                <div className="thp-how__step-num">{s.step}</div>
                <div className="thp-how__step-icon"><div className="icon-box" /></div>
                <h4>{s.title}</h4>
                <p>{s.desc}</p>
              </div>
            ))}
        ...
        <section className="thp-cta">
          ...
          <p>Создай свой маршрут или найди идеальный от наших гидов</p>
          <div className="thp-cta__btns">
            <button className="thp-cta__btn thp-cta__btn--primary" onClick={() => navigate("/traveler/search")}>
              <div className="icon-box" /> Найти маршрут
            </button>
            <button className="thp-cta__btn thp-cta__btn--outline" onClick={() => navigate("/traveler/create-route")}>
              <div className="icon-box" /> Создать свой
            </button>
          </div>
        </section>
